...
from src.cli import ovat

def test_cli_help():
    """Test that the CLI help command works"""
    runner = CliRunner()
    result = runner.invoke(ovat, ['--help'])
    assert result.exit_code == 0
    assert 'OVAT - Open Policy Agent Verification and Testing CLI tool' in result.output

def test_server_help():
    """Test that the server help command works"""
    runner = CliRunner()
    result = runner.invoke(ovat, ['server', '--help'])
    assert result.exit_code == 0
...
def test_server_start_dev_help():
    ...
    runner = CliRunner()
    result = runner.invoke(ovat, ['server', 'start-dev', '--help'])
    assert result.exit_code == 0
